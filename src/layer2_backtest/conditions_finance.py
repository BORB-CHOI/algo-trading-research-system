...
NaN 비교는 False 가 되어 자동 탈락한다. 그래서 두 가지를 조심한다.

- **요약 테이블**(`data/derived/financials.parquet`)이 없으면 전 종목이 빠진다. 종목별 파일을
  받은 뒤 요약을 만들어야 조건이 본다. 날마다 갱신이 사업보고서를 새로 받은 회차에 다시
  만든다(손으로는 `scripts/build_financials.py`).
- 요약이 있어도 파일이 없는 종목은 빠진다. 얼마나 빠지는지는 DATA_SCHEMA §4 에 실측으로 적는다.
...
def coverage() -> dict:
    ...
    요약 테이블이 없으면 `ready=False` 다 — 그때 재무 조건은 전 종목이 빠진다.
    """
