...
1단계 `exclusions.py` 와 역할이 다르다.

- 1단계 = **애초에 거래 대상이 아닌 것** (스팩·KONEX·우선주·리츠·관리종목).
  전략을 어떻게 바꾸든 대상이 아니다. 거의 안 바뀐다.
- 2단계(여기) = **전략상 이런 종목을 노린다**. 거래대금·시총·등락률 조건.
  튜닝하면서 계속 바뀐다. Validate 구간에서 조정하는 대상.

지침서가 §4.2를 layer3(지금 2단계)로 둔 이유가 이것이다 — 여기 숫자는 전략 그 자체다.
