...
- **수정주가 보정(ADR-0006) 적용**: 패널에 Stocks(상장주식수)가 있으면 액면분할/병합을
  ...
  판정 임계값은 1단계 `adjust.py` 와 동일 상수를 쓴다(정본 하나).
...
from src.layer1_market_data.adjust import SPLIT_PRICE_MATCH, SPLIT_SHARE_HI, SPLIT_SHARE_LO
from src.layer2_backtest.conditions_finance import FINANCE_KEYS, FINANCE_SPECS
from src.layer2_backtest.conditions_finance import coverage as finance_coverage
...
class HistPanel:
    ...
    def _adj(self) -> pd.DataFrame | str:
        """액면분할/병합 back-adjust 계수 (ADR-0006, 1단계 adjust.py 와 같은 판정).
