...
#
# ⚠️ 이 값과 `WORKERS` 가 지금 갱신에서 가장 오래 걸리는 두 단계(수급·거래원)를 정한다.
#    ADR-0012 를 잴 때는 호출마다 연결을 새로 맺느라 콜당 145ms 가 그냥 날아갔다.
#    2026-08-28 에 연결을 이어 쓰게 고쳐(통신 85ms → 17ms) 다시 쟀다 — 거래원 조회
#    600콜씩:
#
#      줄기  간격    초당    실패  재시도
#        3   0.5     5.88     0     —      ← 예전 값
#        5   0.2    19.65     0    33 (5.5%)   ← 지금 값
#        8   0.15   27.12     0    93 (15%)
#       10   0.1    30.70     2   143 (24%)
#       14   0.1    31.32     0   187 (31%)
#
#    KIS 문서 한도는 초당 20건이다. 5줄기·0.2초가 딱 그 언저리이고 실패가 0이었다.
#    그 위는 거절을 재시도로 억지로 메우는 구간이라 실패가 섞이기 시작한다.
#    오너가 고르라고 맡겨서(2026-08-29) **한도 안에서 가장 빠른 5줄기·0.2초**를 골랐다.
#    한도를 넘겨 쓰다 막히는 위험을 지면서 얻는 게 40%뿐이라 넘기지 않았다.
#    바꾸려면 여기 둘만 고치면 수급·거래원에 같이 먹는다.
POLICY = CallPolicy(min_interval_sec=0.2, max_attempts=5, backoff_base_sec=2.0)
