...
def client() -> KisClient:
    """줄기마다 클라이언트 하나 — 연결을 계속 쓴다. 몫이 바뀌면 새로 만든다."""
    got = getattr(_LOCAL, "client", None)
    if got is None or getattr(_LOCAL, "lane", "") != _LANE:
        got = KisClient(*_CREDS, policy=lane()[1])
        _LOCAL.client = got
        _LOCAL.lane = _LANE
    return got
...
def collect_day_vi(codes: list[str], day: str) -> tuple[int, int]:
    ...
    def one(code: str) -> None:
        try:
            got = state.fetch_vi(client(), code, day)
        except KisApiError:
            with lock:
                errors[0] += 1
            return
    ...
    with ThreadPoolExecutor(max_workers=lane()[0]) as pool:
        list(pool.map(one, codes))
    frame = state.to_frame(rows)
    if not frame.empty:
        parquet_io.save(frame, OUT_DIR / "vi" / f"{day}.parquet")
...
def collect_flags(codes: list[str], day: str) -> tuple[int, int]:
    ...
    def one(code: str) -> None:
        try:
            got = state.fetch_flags(client(), code)
...
def collect_overtime(codes: list[str], day: str) -> tuple[int, int]:
    ...
    def one(code: str) -> None:
        try:
            got = state.fetch_overtime(client(), code)
        except Exception:  # noqa: BLE001
            with lock:
                errors[0] += 1
            return
...
def main() -> int:
    ...
    if not days:
        ...
        return 1
    left = [d for d in days if not notes.done(f"vi:{d}")]
    print(
    ...
    for i, day in enumerate(left, 1):
        n, err = collect_day_vi(codes, day)
        notes.mark(f"vi:{day}", {"done": True, "rows": n, "errors": err,
                                 "at": datetime.now().isoformat(timespec="seconds")})
        el = time.time() - t0
