...
# ─────────────────────────────────────────────────────────────
# 사이클 저점 (ADR-0012) — 피보나치 시작점.
# 기본 사이클: 10,000 → 고점 20,000 → 폭락 9,000(-55%) → 반등 확정(≥13,500) → 재상승.
# drop_pct=50: 하락 확정 = 고점×0.5 이하, 반등 확정 = 저점×1.5 이상.
# 상승은 두 봉으로 쪼갠다 — 한 봉(저가 10,000·고가 20,000)이면 저가가 자기 고가의 정확히
# 절반이라 같은 봉에서 하락 확정이 걸린다(KRX ±30% 상하한가에선 불가능한 합성 봉의 함정).
# ─────────────────────────────────────────────────────────────
V_BOTTOM: list[Bar] = [
    flat_bar(10_000),
    rally_bar(10_000, 14_000),
    rally_bar(14_000, 20_000),  # idx2 고점 20,000
    drop_bar(20_000, 12_000),  # 저가 12,000 > 10,000 — 아직 하락 확정 아님
    drop_bar(12_000, 9_000),  # idx4 저가 9,000 ≤ 10,000 → 하락 확정, 바닥
    rally_bar(9_000, 14_000),  # 고가 14,000 ≥ 13,500 → 반등 확정, 사이클 시작 = idx4
    rally_bar(14_000, 18_000),
]


class TestFindCycleLow:
    def test_V자_바닥이_사이클_시작이다(self) -> None:
        df = make_df(V_BOTTOM)
    ...
    def test_하락이_없으면_구간_최저_Low_로_대신한다(self) -> None:
        ...
        df = make_df(BASE + RALLY)
        c = find_cycle_low(df, drop_pct=50.0)
        assert c.price == 10_000.0
        assert c.date == df["Date"].iloc[0]  # 최저 동률 → 가장 이른 날
        assert not c.confirmed

    def test_사이클이_여러_번이면_가장_최근_저점이다(self) -> None:
        """반등 확정 후 고점은 새로 잰다 — 두 번째 하락은 새 고점 18,000 기준 -50% 다."""
        two = V_BOTTOM + [
            drop_bar(18_000, 10_000),  # 저가 10,000 > 9,000 — 아직
            drop_bar(10_000, 8_800),  # idx8 저가 8,800 ≤ 18,000×0.5 → 두 번째 하락 확정
            rally_bar(8_800, 13_500),  # 13,500 ≥ 8,800×1.5=13,200 → 사이클 시작 갱신
        ]
        df = make_df(two)
        c = find_cycle_low(df, drop_pct=50.0)
        assert (c.date, c.price) == (df["Date"].iloc[8], 8_800.0)
        assert c.confirmed

    def test_as_of_로_과거_시점을_재현한다(self) -> None:
        """두 번째 폭락 전 시점에서 보면 첫 바닥이 사이클 시작이다."""
        two = V_BOTTOM + [drop_bar(18_000, 8_800), rally_bar(8_800, 13_500)]
        df = make_df(two)
        c = find_cycle_low(df, drop_pct=50.0, as_of=df["Date"].iloc[6])
        assert (c.date, c.price) == (df["Date"].iloc[4], 9_000.0)

    def test_저가_동률은_가장_이른_날이다(self) -> None:
        bars = V_BOTTOM[:5] + [drop_bar(9_500, 9_000)] + [rally_bar(9_000, 14_000)]
        df = make_df(bars)
        c = find_cycle_low(df, drop_pct=50.0)
        assert c.date == df["Date"].iloc[4]  # idx5 도 저가 9,000 이지만 strict 비교라 안 갱신

    def test_경계값은_이상으로_판정한다(self) -> None:
        """정확히 -50% / +50% 도 확정이다 — 경계 미포함이면 '딱 반토막' 종목이 조용히 빠진다."""
        bars = [
            flat_bar(20_000),
            drop_bar(20_000, 10_000),  # 정확히 고점×0.5
            rally_bar(10_000, 15_000),  # 정확히 저점×1.5
        ]
        df = make_df(bars)
        c = find_cycle_low(df, drop_pct=50.0)
        assert (c.date, c.price) == (df["Date"].iloc[1], 10_000.0)
        assert c.confirmed

    def test_drop_pct_범위_검증(self) -> None:
        df = make_df(V_BOTTOM)
        for bad in (0, 100, -5, 150):
            with pytest.raises(ValueError, match="drop_pct"):
                find_cycle_low(df, drop_pct=bad)

    def test_기본값이_없다(self) -> None:
        with pytest.raises(TypeError):
            find_cycle_low(make_df(V_BOTTOM))  # type: ignore[call-arg]
